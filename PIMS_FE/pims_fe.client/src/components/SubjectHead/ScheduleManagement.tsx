...
interface ScheduleFormProps {
  ...
  onClose: () => void;
  onSubmit: (
    dto: CreateDefenseScheduleDto | UpdateDefenseScheduleDto,
  ) => Promise<void>;
  loading: boolean;
  councils: CouncilDto[];
  groups: GroupInfo[];
...
const ScheduleFormModal: React.FC<ScheduleFormProps> = ({
      ...
      if (editData) {
        setForm({
          ...
          startTime: editData.startTime
            ? editData.startTime.substring(0, 5)
            : "",
  ...
  const filteredCouncils = councils.filter(
    (c) => filterSemesterId === "" || c.semesterId === filterSemesterId,
  );
  ...
  const selectCls = (k: keyof FormState) =>
    `w-full px-3 py-2.5 text-sm rounded-xl border outline-none focus:ring-2 focus:ring-primary/30 transition-all ${
      errors[k]
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-[#dbdfe6] sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-xl ${isEdit ? "bg-amber-50" : "bg-primary/10"}`}
            >
              <span
                className={`material-symbols-outlined text-xl ${isEdit ? "text-amber-600" : "text-primary"}`}
              >
            ...
            <div>
              ...
              <p className="text-xs text-[#616f89]">
                {isEdit
                  ? "Update date, time and council for this session"
                  : "Set date, time and council for a group"}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, councilId: e.target.value }))
              }
              className={selectCls("councilId")}
            >
            ...
            {errors.councilId && (
              <p className="text-xs text-red-500 mt-1">{errors.councilId}</p>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, defenseDate: e.target.value }))
              }
              className={selectCls("defenseDate")}
            />
            {errors.defenseDate && (
              <p className="text-xs text-red-500 mt-1">{errors.defenseDate}</p>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-xl text-white disabled:opacity-60 flex items-center justify-center gap-2 ${isEdit ? "bg-amber-600 hover:bg-amber-700" : "bg-primary hover:bg-primary/90"}`}
            >
              {loading && (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#616f89] mt-0.5">
              {schedule.groupName} · {fmtDate(schedule.defenseDate)}
            </p>
          </div>
        </div>
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2.5 text-sm rounded-xl border border-[#dbdfe6] focus:border-primary transition-all underline-none"
          >
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => onSave(roomId ? Number(roomId) : null)}
            disabled={loading}
            className="flex-1 py-2.5 text-sm font-semibold rounded-xl bg-primary text-white hover:bg-primary/90 flex items-center justify-center gap-2"
          >
            {loading && (
              <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-5">
        ...
        <div className="bg-red-50 border border-red-100 rounded-xl p-4 text-sm text-red-700">
          Are you sure you want to delete the defense schedule for{" "}
          <strong>{schedule.groupName}</strong> on{" "}
          <strong>{fmtDate(schedule.defenseDate)}</strong>?
        </div>
        <div className="flex gap-3">
          <button
            onClick={onClose}
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex-1 py-2.5 text-sm font-semibold rounded-xl bg-red-600 text-white hover:bg-red-700 disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {loading && (
              <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            )}
...
const BulkCreateModal: React.FC<BulkCreateModalProps> = ({
  ...
  const previewSlots = React.useMemo(() => {
    ...
    const slot = slotMinutes
      ? Number(slotMinutes)
      : Math.floor(totalMin / selectedGroupIds.length);
  ...
  const validate = () => {
    ...
    if (slotMinutes && isNaN(Number(slotMinutes)))
      e.slotMinutes = "Must be a number";
    setErrors(e);
    return Object.keys(e).length === 0;
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-[#dbdfe6] sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#111318]">
                Bulk Schedule Defense Sessions
              </h2>
              <p className="text-xs text-[#616f89]">
                Assign multiple groups to a council with auto-split time slots
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className={inputCls("defenseDate")}
              />
              {errors.defenseDate && (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className={inputCls("windowStart")}
              />
              {errors.windowStart && (
                <p className="text-xs text-red-500 mt-1">
                  {errors.windowStart}
            ...
            <div>
              ...
              <input
                ...
                className={inputCls("windowEnd")}
              />
              {errors.windowEnd && (
                <p className="text-xs text-red-500 mt-1">{errors.windowEnd}</p>
              )}
            </div>
            <div>
              <label className="block text-sm font-semibold text-[#111318] mb-1.5">
                Slot (min){" "}
                <span className="text-[#b0b8c9] font-normal text-xs">
                  optional
                </span>
              </label>
              <input
                ...
                className={inputCls("slotMinutes")}
              />
              {errors.slotMinutes && (
                <p className="text-xs text-red-500 mt-1">
                  {errors.slotMinutes}
          ...
          {/* Group Selection */}
          <div>
            <label className="block text-sm font-semibold text-[#111318] mb-2">
              Select Groups <span className="text-red-500">*</span>
            </label>
            {errors.groups && (
              <p className="text-xs text-red-500 mb-2">{errors.groups}</p>
            )}
            <div className="border border-[#dbdfe6] rounded-xl overflow-hidden">
              <div className="bg-[#f8f9fb] px-3 py-2 border-b border-[#dbdfe6] flex items-center justify-between">
                <span className="text-xs font-bold text-[#616f89] uppercase tracking-wide">
              ...
              <div className="max-h-48 overflow-y-auto divide-y divide-[#f0f2f5]">
                {groups.map((g) => (
                  <label
                    key={g.groupId}
                    className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-[#f8f9fb] transition-colors"
                  >
                    <input
                      type="checkbox"
                ...
                {previewSlots.map((s, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="w-5 h-5 bg-indigo-200 text-indigo-700 rounded-md flex items-center justify-center font-black text-[10px]">
                        {i + 1}
                      </span>
                    ...
                    <span className="font-mono font-bold text-indigo-600 bg-white px-2 py-0.5 rounded-lg border border-indigo-100">
                      {s.start} – {s.end}
                    </span>
                  </div>
                ))}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 text-sm font-semibold rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-60 flex items-center justify-center gap-2"
            >
              {loading && (
                <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              )}
...
const ScheduleManagement: React.FC = () => {
  ...
  const [assignTarget, setAssignTarget] = useState<DefenseScheduleDto | null>(
    null,
  );
  ...
  const [deleteTarget, setDeleteTarget] = useState<DefenseScheduleDto | null>(
    null,
  );
  ...
  const [assessments, setAssessments] = useState<
    { assessmentId: number; title: string }[]
  >([]);
  ...
  const fetchSupport = useCallback(async () => {
    try {
      ...
      if (sem) {
        ...
        const eligibleRes = await defenseScheduleService.getEligibleGroups(
          sem.semesterId,
        );
  ...
  const fetchAssessments = useCallback(async () => {
    try {
      ...
      const finalAssessments = (res.data ?? []).filter((a) =>
  ...
  const fetchGroups = useCallback(
    async (retake: boolean, assessmentId: string, semId?: number) => {
      try {
        if (retake && assessmentId && semId) {
          const res = await groupService.getGroupsEligibleForRetake(
            semId,
            Number(assessmentId),
          );
  ...
  const handleCreate = async (dto: CreateDefenseScheduleDto) => {
    ...
    try {
      ...
      if (activeSemesterId) {
        const eligibleRes =
          await defenseScheduleService.getEligibleGroups(activeSemesterId);
        setGroups(eligibleRes.data ?? []);
      }
    } catch (err: any) {
  ...
  const handleBulkCreate = async (dto: BulkCreateDefenseScheduleDto) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message ?? "Failed to bulk create schedules",
      );
    } finally {
  ...
  const getDaySessions = (day: Date) => {
    return filteredSchedules.filter(
      (s) => s.defenseDate && isSameDay(parseISO(s.defenseDate), day),
    );
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#616f89] mt-1 text-sm">
            Manage defense sessions for students.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-[#616f89] text-sm animate-pulse">
            ...
            <div className="p-6 flex items-center justify-between border-b border-[#dbdfe6] bg-[#f8f9fb]">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 rounded-xl hover:bg-[#ebedf2] border border-[#dbdfe6] bg-white transition-all text-[#616f89]"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-[#dbdfe6] bg-white text-xs font-bold hover:bg-[#ebedf2] transition-all text-[#111318]"
                >
                  Today
                </button>
                <button
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="grid grid-cols-7 border-b border-[#dbdfe6] bg-[#f8f9fb]">
                {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
                  <div
                    key={d}
                    className="py-2 text-center text-[10px] font-black text-[#616f89] uppercase tracking-widest"
                  >
                    {d}
                  </div>
                ))}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span
                          className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-lg ${isToday(day) ? "bg-primary text-white shadow-md shadow-primary/30" : activeDate ? "text-primary bg-primary/10" : "text-[#111318]"}`}
                        >
                          {format(day, "d")}
                        </span>
                      </div>
                      <div className="space-y-1">
                        {sessions.slice(0, 3).map((s) => (
                          <div
                            key={s.scheduleId}
                            className="px-1.5 py-0.5 rounded-md bg-primary/5 border-l-2 border-primary text-[9px] font-bold text-primary truncate"
                          >
                            {s.groupName || "Group"}
                          </div>
                        ))}
                        {sessions.length > 3 && (
                          <div className="text-[8px] font-black text-[#616f89] pl-2">
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                {selectedDaySessions.length > 0 ? (
                  selectedDaySessions.map((s) => (
                    <div
                      key={s.scheduleId}
                      className="p-4 rounded-2xl border border-[#f0f2f5] hover:border-primary/30 transition-all bg-[#f8f9fb] group"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span
                          className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${statusBadge(s.status)}`}
                        >
                          {s.status}
                        </span>
                        <span className="text-[10px] font-bold text-[#616f89] bg-white px-2 py-1 rounded-lg border border-[#f0f2f5]">
                          {fmtTime(s.startTime)}
                        </span>
                      </div>
                      <h4 className="font-bold text-[#111318] text-sm group-hover:text-primary transition-colors">
                        {s.groupName || `Group #${s.groupId}`}
                      </h4>
