...
const TeacherCouncilGradingPage: React.FC = () => {
  ...
  const [allSemesterAssessments, setAllSemesterAssessments] = useState<
    AssessmentWithCriteriaDto[]
  >([]);
  ...
  const fetchCouncilDetails = useCallback(async () => {
    ...
    try {
      ...
      let targetCouncilId: number | null = councilId
        ? parseInt(councilId)
        : null;
      ...
      const allSchedulesRes = await defenseScheduleService.getAll(
        councilRes.data.semesterId,
      );
      const allSemesterSchedules = allSchedulesRes.data ?? [];
      setAllSchedules(allSemesterSchedules);

      const councilSchedules = allSemesterSchedules.filter(
        (s) => s.councilId === targetCouncilId,
      );
      setSchedules(councilSchedules);

      if (councilRes.data) {
        const assessmentsRes =
          await assessmentService.getAssessmentsWithCriteria(
            councilRes.data.semesterId,
            true,
          );
        const allAss = assessmentsRes.data ?? [];
        setAllSemesterAssessments(allAss);
        const finalAssessments = allAss
          .filter((a) => a.isFinal)
      ...
      if (councilSchedules.length > 0) {
        ...
        if (dates.length > 0) {
          ...
          const firstGroup = councilSchedules.find(
            (s) => s.defenseDate?.toString() === dates[0],
          );
  ...
  useEffect(() => {
    const fetchGroupData = async () => {
      ...
      try {
        ...
        let existingScoresMap: Record<
          number,
          Record<number, Record<number, number>>
        > = {};
        const gradePromises = groupScheds.map(async (sched) => {
          existingScoresMap[sched.scheduleId] = {};
          try {
            const res = await assessmentService.getCouncilGrades(
              sched.councilId,
              selectedGroupId,
            );
            (res.data ?? []).forEach((g: any) => {
              ...
              if (uId !== undefined && crId !== undefined) {
                if (!existingScoresMap[sched.scheduleId][uId])
                  existingScoresMap[sched.scheduleId][uId] = {};
                existingScoresMap[sched.scheduleId][uId][crId] =
                  g.score ?? g.Score ?? 0;
              }
            });
          } catch (e) {
        ...
        const initialScores: Record<
          ...
          Record<number, Record<number, number>>
        > = {};
        groupScheds.forEach((sched, idx) => {
          const isRetake = idx > 0;
          const assConfig = isRetake
            ? allSemesterAssessments.find((a) => a.isRetake)
            : assessments[0];
          if (!assConfig || !assConfig.criteria) return;

          initialScores[sched.scheduleId] = {};
          members.forEach((m) => {
            initialScores[sched.scheduleId][m.userId] = {};
            assConfig.criteria.forEach((c) => {
              // Default to 0, which means no score visually if it was truly 0 or hasn't been scored.
              initialScores[sched.scheduleId][m.userId][c.criteriaId] =
                existingScoresMap[sched.scheduleId]?.[m.userId]?.[
                  c.criteriaId
                ] ?? 0;
            });
          });
  ...
  const handleScoreChange = (
    ...
    setStudentScores((prev) => ({
      ...
      [schedId]: {
        ...prev[schedId],
        [uId]: { ...prev[schedId]?.[uId], [critId]: clamped },
      },
    }));
  };
  ...
  const calculateStudentTotal = (
    ...
    const assessment = isRetake
      ? allSemesterAssessments.find((a) => a.isRetake)
      : assessments[0];
    if (!assessment || !assessment.criteria) return 0;
  ...
  const handleSubmit = async (scheduleId: number, isRetake: boolean) => {
    const baseAss = assessments[0];
    let targetAssessment = baseAss;

    if (isRetake) {
      const retakeAss = allSemesterAssessments.find((a) => a.isRetake);
      if (retakeAss) {
        targetAssessment = retakeAss;
      } else {
        toast.error("Retake grading criteria not configured.");
        return;
      }
    }

    if (!council || !selectedGroupId || !targetAssessment) return;
    setSubmitting(true);
    try {
      const studentScoresDto: CouncilStudentScoreDto[] = Object.entries(
        studentScores[scheduleId] || {},
      )
        .filter(([uIdStr]) => {
          const uId = parseInt(uIdStr);
          return (
            groupMembers.some((m) => m.userId === uId) &&
            !(isRetake && passedUserIds.includes(uId))
          );
        })
  ...
  const availableDates = Array.from(
    new Set(schedules.map((s) => s.defenseDate?.toString()).filter(Boolean)),
  ) as string[];
  availableDates.sort();
  ...
  const filteredSchedules = selectedDate
    ...
    : schedules;
  const groupSchedulesForThisGroup = allSchedules
    .filter((s) => s.groupId === selectedGroupId)
    .sort(
      (a, b) =>
        new Date(a.defenseDate || "").getTime() -
  ...
  const currentScheduleInContext = filteredSchedules.find(
    (s) => s.groupId === selectedGroupId,
  );
  const currentScheduleInGroupIdx = groupSchedulesForThisGroup.findIndex(
    (s) => s.scheduleId === currentScheduleInContext?.scheduleId,
  );
          ...
          {loading ? (
            ...
            <div className="max-w-md w-full mx-auto bg-white rounded-3xl border border-[#dbdfe6] p-10 text-center space-y-6 shadow-sm">
              ...
              <p className="text-sm text-gray-500">
                You are not assigned to any defense council this semester.
              </p>
            </div>
          ) : (
              ...
              <div className="xl:col-span-3 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10">
                      ...
                      <p className="text-sm font-bold text-gray-800">
                        {currentScheduleInContext?.roomName || "No Room"}
                      </p>
                      {currentScheduleInContext?.defenseDate && (
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-[#616f89] uppercase tracking-wider mb-2">
                        Members
                      </p>
                      <div className="space-y-2">
                        {council.members.map((m) => (
                          ...
                          >
                            <div className="size-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-[10px]">
                              {m.fullName.charAt(0)}
                            </div>
                ...
                {myCouncils.length > 1 && (
                  <div className="bg-white rounded-3xl border border-[#dbdfe6] p-4 shadow-sm">
                    <p className="text-[10px] font-bold text-[#616f89] uppercase tracking-wider mb-3 px-2">
                      Switch Council
                    </p>
                    <select
                      value={council.councilId}
                      onChange={(e) =>
                        navigate(`/teacher/council-grading/${e.target.value}`)
                      }
                      className="w-full h-10 px-3 bg-gray-50 border border-[#dbdfe6] rounded-xl text-xs font-bold outline-none focus:border-primary"
                    >
                      {myCouncils.map((c) => (
                    ...
                    <div className="flex flex-wrap gap-2 px-1">
                      {availableDates.map((d) => (
                        <button
                          ...
                          onClick={() => {
                            setSelectedDate(d);
                            const f = schedules.find(
                              (sx) => sx.defenseDate?.toString() === d,
                            );
                            if (f) setSelectedGroupId(f.groupId);
                          }}
                          className={`px-3 py-2 rounded-xl text-[10px] font-bold transition-all ${selectedDate === d ? "bg-primary text-white shadow-md" : "bg-gray-50 text-gray-500 hover:bg-gray-100"}`}
                        >
                  ...
                  <div className="max-h-[400px] overflow-y-auto">
                    {filteredSchedules.map((s) => (
                      <button
                        ...
                        className={`w-full p-4 flex items-center justify-between border-l-4 transition-all ${selectedGroupId === s.groupId ? "bg-primary/5 border-primary" : "border-transparent hover:bg-gray-50"}`}
                      >
                        <div className="text-left">
                          <p
                            className={`text-sm font-bold ${selectedGroupId === s.groupId ? "text-primary" : "text-gray-700"}`}
                          >
                            {s.groupName}
                          </p>
                ...
                {!selectedGroupId ? (
                  <div className="flex flex-col items-center justify-center min-h-[600px] bg-white rounded-3xl border-2 border-dashed border-[#dbdfe6] text-center p-12">
                    ...
                    <p className="text-gray-500 max-w-sm">
                      Pick a group from the list to start evaluating performance
                      results.
                    </p>
                  </div>
                ) : (
                  <>
                    {currentScheduleInContext?.status?.toUpperCase() ===
                      "COMPLETED" && (
                      <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center gap-3 text-emerald-800">
                        ...
                        <p className="text-sm font-medium">
                          Grading for this group is completed. Scores can no
                          longer be modified.
                        </p>
                      </div>
                    )}
                    <div className="bg-white rounded-3xl border border-[#dbdfe6] p-8 shadow-sm space-y-10 min-h-[600px]">
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                            <h2 className="text-2xl font-black text-gray-900">
                              {currentScheduleInContext?.groupName}
                            </h2>
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${currentScheduleInContext?.status?.toUpperCase() === "COMPLETED" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                            >
                              {currentScheduleInContext?.status || "PENDING"}
                            </span>
                          </div>
                          <p className="text-sm text-gray-500 flex items-center gap-2">
                            ...
                            Evaluate performance. Scores are averaged among
                            council members.
                          </p>
                        </div>
                        <div className="flex flex-col gap-3 min-w-[240px]">
                          {groupSchedulesForThisGroup.map((sched, idx) => {
                            if (sched.councilId !== council.councilId)
                              return null;
                            // Only show save button for the currently selected schedule context
                            if (
                              selectedDate &&
                              sched.defenseDate?.toString() !== selectedDate
                            )
                              return null;
                            ...
                            const isCompleted =
                              sched.status?.toUpperCase() === "COMPLETED";
                            return (
                              <button
                                key={`save-${sched.scheduleId}`}
                                onClick={() =>
                                  handleSubmit(sched.scheduleId, isRetake)
                                }
                                disabled={submitting || expired || isCompleted}
                                className="w-full px-6 py-2.5 bg-primary text-white text-sm font-bold rounded-xl shadow-md hover:bg-primary/90 transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                              >
                                {submitting ? (
                                ...
                                {submitting
                                  ? "Saving..."
                                  : isRetake
                                    ? "Save Attempt 2 (Retake)"
                                    : "Save Attempt 1 (Final)"}
                              </button>
                            );
                          })}
                        </div>
                      ...
                      {assessments.length === 0 ? (
                        <div className="p-12 bg-amber-50 rounded-2xl border border-amber-100 text-center text-amber-800">
                          ...
                          <p className="text-sm">
                            No grading criteria for Final assessments was found.
                          </p>
                        </div>
                      ) : groupLoading ? (
                        ...
                        <div className="space-y-12">
                          {groupMembers
                            ...
                            .map((member) => (
                              <div
                                key={member.userId}
                                className="p-1 rounded-[2.5rem] hover:bg-primary/5 transition-all group"
                              >
                                <div className="bg-white border border-[#dbdfe6] rounded-[2.3rem] overflow-hidden shadow-sm group-hover:border-primary/20 transition-all">
                                  ...
                                  <div className="p-8 space-y-8">
                                    {groupSchedulesForThisGroup.map(
                                      (sched, idx) => {
                                        if (
                                          sched.scheduleId !==
                                          currentScheduleInContext?.scheduleId
                                        )
                                          return null;
                                        ...
                                        const isMyCouncil =
                                          sched.councilId === council.councilId;
                                        ...
                                        return (
                                          <div
                                            key={`box-${sched.scheduleId}`}
                                            className={`p-6 rounded-3xl border ${isLocked ? (expired ? "bg-rose-50/30 border-rose-100" : "bg-emerald-50/30 border-emerald-100") : "bg-gray-50/30 border-[#dbdfe6]"}`}
                                          >
                                            <div className="flex justify-between items-start mb-6">
                                              <div className="space-y-2">
                                                <h5 className="font-black text-gray-800 text-xs uppercase tracking-widest flex items-center gap-2">
                                                  ...
                                                  {isRetake
                                                    ? "FINAL (ATTEMPT 2 - RETAKE)"
                                                    : "FINAL (ATTEMPT 1)"}
                                                </h5>
                                                <div className="flex gap-2">
                                                  {isRetake && isPassedL1 && (
                                                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[9px] font-black rounded-lg uppercase">
                                                      Passed Attempt 1
                                                    </span>
                                                  )}
                                                  {expired && (
                                                    <span className="px-2 py-0.5 bg-rose-100 text-rose-700 text-[9px] font-black rounded-lg uppercase">
                                                      Deadline Passed
                                                    </span>
                                                  )}
                                                  {!isMyCouncil && !expired && (
                                                    <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-[9px] font-black rounded-lg uppercase">
                                                      View Only
                                                    </span>
                                                  )}
                                                </div>
                                              ...
                                              <div className="text-right">
                                                ...
                                                <p
                                                  className={`text-2xl font-black ${calculateStudentTotal(sched.scheduleId, member.userId, isRetake) < 4 ? "text-rose-500" : "text-primary"}`}
                                                >
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                                              {(isRetake
                                                ...
                                                >
                                                  <label
                                                    className="text-[10px] font-bold text-gray-500 block truncate"
                                                  ...
                                                  <input
                                                    ...
                                                    className={`w-full h-11 px-4 bg-white border border-[#dbdfe6] rounded-xl font-bold text-gray-900 outline-none transition-all ${isLocked ? "bg-gray-100/50 cursor-not-allowed text-gray-400" : "focus:border-primary focus:ring-4 focus:ring-primary/10 hover:border-primary/40 shadow-sm"}`}
                                                    placeholder="0.0"
                                                  />
                                                </div>
                                              ))}
