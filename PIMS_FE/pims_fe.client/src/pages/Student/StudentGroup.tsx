...
import { CloudUpload, FileText, Info, Loader2, SendHorizontal, AlertCircle } from 'lucide-react';
...
const StudentGroup = () => {
    ...
    const handleSubmit = async () => {
        ...
        // Giả lập gọi API (Bạn sẽ thay thế bằng axios.post ở đây)
        try {
            await new Promise(resolve => setTimeout(resolve, 2000));

            // Giả lập lỗi Foreign Key ProjectId mà bạn đang gặp
            const mockError = false;

            if (mockError) {
                throw new Error('FK_Submission_Project');
            }

            toast.success('Nộp báo cáo thành công lên Google Drive!');
            setSelectedFile(null);
        } catch (error) {
            if (error.message.includes('FK_Submission_Project')) {
                toast.error('Nhóm của bạn chưa được phân công dự án hoặc không hợp lệ. Vui lòng liên hệ Mentor!', { duration: 5000 });
            } else {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <ul className="space-y-4">
                            <li className="flex gap-4 pb-4 border-b border-gray-50 last:border-0">
                                ...
                                <p className="text-gray-600 text-sm">Hệ thống bảo trì từ 23:00 - 01:00 đêm nay để nâng cấp máy chủ.</p>
                            </li>
                            <li className="flex gap-4 pb-4 border-b border-gray-50 last:border-0">
                                <div className="w-2 h-2 mt-2 bg-[#10b981] rounded-full shrink-0"></div>
                                <p className="text-gray-600 text-sm">TS. Nguyễn Văn A đã duyệt báo cáo Iteration 1. Nhóm 07 đạt kết quả Tốt.</p>
                            </li>
                        </ul>
                    </section>

                    {/* Card C: Nộp báo cáo - TRỌNG TÂM */}
                    <section className="bg-white p-8 rounded-2xl shadow-md border-t-4 border-t-[#10b981]">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="w-full bg-[#10b981] text-white py-4 rounded-xl font-bold text-lg shadow-lg shadow-green-100 hover:bg-[#0da673] active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                        >
                    ...
                    <section className="bg-slate-50 p-8 rounded-2xl border border-slate-200">
                        ...
                        <ul className="space-y-4 text-slate-600">
                            <li className="flex items-start gap-3">
                                ...
                                <span>Đảm bảo bạn đã chọn <strong>đúng cột mốc</strong> Iteration hiện tại.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                ...
                                <span>Hệ thống chỉ chấp nhận file nén định dạng <strong>.zip, .rar hoặc .7z</strong>.</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <span className="w-6 h-6 bg-slate-200 text-slate-800 rounded-full flex items-center justify-center text-xs font-bold shrink-0">3</span>
                                <span>Đặt tên file mẫu: <code className="bg-slate-200 px-1 rounded text-sm font-bold">[MSSV]_[TenNhom]_[Ky]</code>.</span>
                            </li>
                        </ul>
                    </section>
