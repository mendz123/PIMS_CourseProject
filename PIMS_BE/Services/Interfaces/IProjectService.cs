...
namespace PIMS_BE.Services.Interfaces
{
    ...
    { 
        ...
        Task<SubmissionHistoryDto> SubmitReportAsync(SubmitProjectReportDto dto, int studentId);
        Task<ApiResponse<List<SubmissionHistoryDto>>> GetSubmissionHistoryAsync(int studentId);
        Task<ApiResponse<SubmissionHistoryDto>> UpdateSubmissionAsync(int submissionId, SubmitProjectReportDto dto, int userId);
